...
public class MapperTest {
	...
	public void testMap() {
		...
		ModelA ma = (ModelA) mapper.mapData(str, ModelA.class);
		...
		ma = (ModelA) mapper.convert(map, ModelA.class);
		assertThat(ma.getName(), is("hello world"));

		//TypeReference<ModelA> t = new TypeReference<ModelA>(){};

		ma = (ModelA) mapper.mapData(str, new TypeReference<ModelA>() {});
		assertThat(ma.getName(), is("hello"));
		assertThat(mapper.dataToString(ma), is(str));

		str = "{\"id\":\"1q2w3e\",\"list\":[{\"name\":\"asd\"},{\"name\":\"zxc\"}]}";
		ModelB<?> mb = (ModelB<?>) mapper.mapData(str, ModelB.class);
		...
		ma = (ModelA) mapper.convert((Map<String, Object>) mb.getList().get(0), ModelA.class);
	...
	public void testWithEnum() {
		...
		ModelWithEnum me = (ModelWithEnum) mapper.mapData(str, ModelWithEnum.class);
		...
		me = (ModelWithEnum) mapper.mapData(str, ModelWithEnum.class);
		assertThat(me.getName(), is("hello"));
